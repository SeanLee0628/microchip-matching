...
class TestParseBlog(unittest.TestCase):
    ...
    def test_cancel_window_excludes_past_and_picks_nearest_future(self):
        # PDD - Change Window(일). today 이상인 날짜만 보고, 그중 가장 가까운 날짜.
        data = [
            ["P1", 100, 10, datetime(2026, 8, 7), 45, 9, 17, "C"],   # 8/7 - 45 = 6/23 (미래)
            ["P1", 100, 10, datetime(2026, 6, 26), 45, 9, 17, "C"],  # 6/26 - 45 = 5/12 (과거 → 제외)
        ]
        out = m5.parse_blog(self._rows(data), today=date(2026, 6, 1))
        # 5/12 는 지난 날짜라 제외, 미래인 6/23 만 남음
        self.assertEqual(out[m5.make_mix(100, "P1")]["cancel_window"], date(2026, 6, 23))

    def test_cancel_window_picks_closest_among_multiple_future(self):
        data = [
            ["P1", 100, 10, datetime(2026, 8, 7), 45, 9, 17, "C"],   # → 6/23
            ["P1", 100, 10, datetime(2026, 9, 10), 45, 9, 17, "C"],  # → 7/27
        ]
        out = m5.parse_blog(self._rows(data), today=date(2026, 6, 1))
        # 둘 다 미래 → 더 가까운(=최소) 6/23
        self.assertEqual(out[m5.make_mix(100, "P1")]["cancel_window"], date(2026, 6, 23))

    def test_cancel_window_none_when_all_past(self):
        data = [
            ["P1", 100, 10, datetime(2026, 8, 7), 45, 9, 17, "C"],   # → 6/23
            ["P1", 100, 10, datetime(2026, 6, 26), 45, 9, 17, "C"],  # → 5/12
        ]
        out = m5.parse_blog(self._rows(data), today=date(2026, 7, 1))
        # 6/23, 5/12 모두 7/1 이전 → 전부 제외 → None
        self.assertIsNone(out[m5.make_mix(100, "P1")]["cancel_window"])

    def test_cancel_window_includes_today_itself(self):
        data = [["P1", 100, 10, datetime(2026, 7, 1), 0, 9, 17, "C"]]  # 7/1 - 0 = 7/1
        out = m5.parse_blog(self._rows(data), today=date(2026, 7, 1))
        # 오늘과 같은 날짜는 포함(>= today)
        self.assertEqual(out[m5.make_mix(100, "P1")]["cancel_window"], date(2026, 7, 1))
...
class TestMonthWindow(unittest.TestCase):
    """월 컬럼(6월…3월)에 연도를 부여한다 — 라벨만 보고 담던 것이 오류의 원인이었다."""

    def test_window_rolls_year_at_january(self):
        win = m5.month_window(2026)
        self.assertEqual(win[(2026, 6)], "6월")
        self.assertEqual(win[(2026, 12)], "12월")
        self.assertEqual(win[(2027, 1)], "1월")
        self.assertEqual(win[(2027, 3)], "3월")
        self.assertEqual(len(win), 10)

    def test_window_excludes_april_and_may(self):
        win = m5.month_window(2026)
        self.assertNotIn((2027, 4), win)
        self.assertNotIn((2027, 5), win)


class TestBlogYearAwareBuckets(unittest.TestCase):
    HEADER = TestParseBlog.HEADER

    def _rows(self, rows, title="260609"):
        return _wb_bytes([self.HEADER], rows, sheet_title=title)

    def test_far_future_not_mixed_into_near_month(self):
        # 2028-06 물량이 '6월'(=2026-06) 칸에 섞이면 안 된다.
        data = [
            ["P1", 100, 3000, datetime(2026, 6, 7), 45, 9, 17, "C"],
            ["P1", 100, 920, datetime(2028, 6, 7), 45, 9, 17, "C"],
        ]
        recs, meta = m5.read_blog(self._rows(data), today=date(2026, 6, 9))
        rec = recs[m5.make_mix(100, "P1")]
        self.assertEqual(rec["monthly"][6], 3000)
        self.assertEqual(rec["out_of_window"], 920)
        self.assertEqual(meta["out_of_window"][(2028, 6)], 920)

    def test_next_year_jan_to_mar_are_in_window(self):
        # 창은 2026-06~2027-03 이므로 2027-01 은 '1월' 칸이 맞다.
        data = [["P1", 100, 5000, datetime(2027, 1, 15), 45, 9, 17, "C"]]
        recs, meta = m5.read_blog(self._rows(data), today=date(2026, 6, 9))
        self.assertEqual(recs[m5.make_mix(100, "P1")]["monthly"][1], 5000)
        self.assertEqual(meta["out_of_window"], {})

    def test_blog_ttl_still_counts_everything(self):
        data = [
            ["P1", 100, 3000, datetime(2026, 6, 7), 45, 9, 17, "C"],
            ["P1", 100, 44000, datetime(2027, 4, 7), 45, 9, 17, "C"],  # 창 밖
        ]
        recs, meta = m5.read_blog(self._rows(data), today=date(2026, 6, 9))
        rec = recs[m5.make_mix(100, "P1")]
        self.assertEqual(rec["blog_ttl"], 47000)          # 총계는 전부 포함
        self.assertEqual(sum(rec["monthly"].values()), 3000)  # 월 칸은 창 안만
        self.assertEqual(meta["qty_total"], 47000)

    def test_asof_read_from_sheet_name(self):
        recs, meta = m5.read_blog(self._rows([["P1", 100, 1, datetime(2026, 6, 7), 45, 9, 17, "C"]]))
        self.assertEqual(meta["asof"], date(2026, 6, 9))
        self.assertEqual(meta["sheet"], "260609")

    def test_asof_none_for_non_date_sheet_name(self):
        data = [["P1", 100, 1, datetime(2026, 6, 7), 45, 9, 17, "C"]]
        _recs, meta = m5.read_blog(self._rows(data, title="Unitron"))
        self.assertIsNone(meta["asof"])

    def test_parse_blog_wrapper_still_returns_records_only(self):
        data = [["P1", 100, 1, datetime(2026, 6, 7), 45, 9, 17, "C"]]
        out = m5.parse_blog(self._rows(data))
        self.assertIn(m5.make_mix(100, "P1"), out)


class TestUnmatchedNotZero(unittest.TestCase):
    """백록에 없는 MIX 를 0 으로 찍으면 매칭 실패가 숫자로 위장된다."""

    def test_mix_absent_from_blog_gets_none_not_zero(self):
        ship = {"100P1": {"담당자": "김", "고객": "A", "고객코드": "100",
                          "part": "P1", "y2023": 0, "y2024": 0, "y2025": 0, "y2026": 10}}
        _c, _d, recs = m5.build_records({}, {}, {}, ship)
        row = recs[0]
        self.assertIsNone(row["BLOG TTL"])
        self.assertIsNone(row["6월"])
        self.assertIsNone(row["3월"])

    def test_mix_present_in_blog_keeps_zero_for_empty_month(self):
        blog = {"100P1": {"lead_time": 9, "cancel_window": None, "blog_ttl": 500,
                          "monthly": {6: 500}, "out_of_window": 0,
                          "더존코드": "100", "더존업체명": "A", "part": "P1"}}
        _c, _d, recs = m5.build_records({}, {}, blog, {})
        row = recs[0]
        self.assertEqual(row["BLOG TTL"], 500)
        self.assertEqual(row["6월"], 500)
        self.assertEqual(row["7월"], 0)   # 백록에 있는데 그 달 물량이 없는 것 = 진짜 0

    def test_balance_unchanged_by_none_display(self):
        # 표시가 '-' 여도 Balance 산식은 종전대로 0 취급
        ship = {"100P1": {"담당자": "김", "고객": "A", "고객코드": "100",
                          "part": "P1", "y2023": 0, "y2024": 0, "y2025": 0, "y2026": 10}}
        _c, _d, recs = m5.build_records({"P1": 700}, {"100P1": {"demand": 200}}, {}, ship)
        self.assertEqual(recs[0]["Balance"], 500)  # 700 + 0 - 200


class TestDiagnostics(unittest.TestCase):
    HEADER = TestParseBlog.HEADER

    def _read(self, rows, title="260609", today=None):
        data = _wb_bytes([self.HEADER], rows, sheet_title=title)
        return m5.read_blog(data, today=today or date(2026, 6, 9))

    def test_stale_blog_file_warns(self):
        recs, meta = self._read([["P1", 100, 10, datetime(2026, 6, 7), 45, 9, 17, "C"]])
        _c, _d, records = m5.build_records({}, {}, recs, {})
        warn = m5.diagnostics(meta, records, today=date(2026, 6, 20))  # 11일 경과
        self.assertTrue(any("오래되었습니다" in w for w in warn), warn)

    def test_fresh_blog_file_no_stale_warning(self):
        recs, meta = self._read([["P1", 100, 10, datetime(2026, 6, 7), 45, 9, 17, "C"]])
        _c, _d, records = m5.build_records({}, {}, recs, {})
        warn = m5.diagnostics(meta, records, today=date(2026, 6, 12))  # 3일 경과
        self.assertFalse(any("오래되었습니다" in w for w in warn), warn)

    def test_totals_reconcile_when_all_in_window(self):
        recs, meta = self._read([["P1", 100, 10, datetime(2026, 6, 7), 45, 9, 17, "C"]])
        _c, _d, records = m5.build_records({}, {}, recs, {})
        warn = m5.diagnostics(meta, records, today=date(2026, 6, 10))
        self.assertFalse(any("합계 불일치" in w for w in warn), warn)

    def test_out_of_window_qty_is_reported_with_detail(self):
        recs, meta = self._read([
            ["P1", 100, 10, datetime(2026, 6, 7), 45, 9, 17, "C"],
            ["P1", 100, 44000, datetime(2027, 4, 7), 45, 9, 17, "C"],
        ])
        _c, _d, records = m5.build_records({}, {}, recs, {})
        warn = m5.diagnostics(meta, records, today=date(2026, 6, 10))
        self.assertTrue(any("기간" in w and "2027-04" in w for w in warn), warn)
        # 총계 자체는 여전히 맞아떨어져야 한다(월합 + 기간밖 = 원본)
        self.assertFalse(any("합계 불일치" in w for w in warn), warn)

    def test_unmatched_rows_are_reported(self):
        recs, meta = self._read([["P1", 100, 10, datetime(2026, 6, 7), 45, 9, 17, "C"]])
        ship = {"200P9": {"담당자": "김", "고객": "B", "고객코드": "200",
                          "part": "P9", "y2023": 0, "y2024": 0, "y2025": 0, "y2026": 5}}
        _c, _d, records = m5.build_records({}, {}, recs, ship)
        warn = m5.diagnostics(meta, records, today=date(2026, 6, 10))
        self.assertTrue(any("백록에 없는 MIX 1행" in w for w in warn), warn)
